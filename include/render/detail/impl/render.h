    ...
    <
      ...
      (
          ...
          [&](auto &&settings_tup) {
            ...
            using TriAccel =
                intersect::accel::AccelT<triangle_accel_type, execution_model,
                                         Triangle>;
            ...
            for (unsigned i = 0; i < num_meshs; i++) {
              if (!ref_set[i]) {
                ...
                cpu_refs[i] = triangle_accels.add(
                    s.triangles(), get_previous(i, s.mesh_ends()),
                    s.mesh_ends()[i], aabb, specific_settings,
                    s.mesh_paths()[i]);
              }
            }
            ...
            constexpr auto term_prob_type =
                compile_time_settings.term_prob_type();
            ...
            auto term_prob =
                term_probs_.template get_item<term_prob_type>().gen(
            ...
            compute_intensities(settings.general_settings.settings, division,
