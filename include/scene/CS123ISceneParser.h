...
public:
  ...
  virtual bool get_camera_data(CS123SceneCameraData &data) const = 0;

  // Returns the number of lights in the scene
  virtual int get_num_lights() const = 0;

  // On return, data will contain the information for the ith light.
  virtual bool get_light_data(const int i, CS123SceneLightData &data) const = 0;

  // On return data will contain the global scene data
  virtual bool get_global_data(CS123SceneGlobalData &data) const = 0;

  // Returns the root node of the scene graph
  virtual CS123SceneNode *get_root_node() const = 0;
