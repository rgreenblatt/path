...
#pragma omp parallel for collapse(2) sched(dynamic, 16)
  for (unsigned block_idx = 0; block_idx < grid; block_idx++) {
    for (unsigned thread_idx = 0; thread_idx < block_size; thread_idx++) {
      initial_world_space_directions_impl(
          block_idx, thread_idx, block_size, division, x_dim, y_dim, accel,
          intermediate_intensities, triangle_data, materials, film_to_world);
    }
  }
