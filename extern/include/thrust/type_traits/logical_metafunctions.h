 ...
 *  \brief C++17's \c conjunction, \c disjunction, and \c negation
 * metafunctions.
 */
...
/// A <code>constexpr bool</code> whose value is <code>(... &&
/// Ts::value)</code>.
...
/// An \c integral_constant whose value is <code>!Ts::value</code>.
template <typename Ts> using negation = std::negation<Ts>;
...
template <bool... Bs> struct conjunction_value;
...
template <> struct conjunction_value<> : std::true_type {};
...
/// An \c integral_constant whose value is <code>(... || Bs)</code>.
template <bool... Bs> struct disjunction_value;
...
template <> struct disjunction_value<> : std::false_type {};
...
template <bool B> struct negation_value;
...
template <bool B> struct negation_value : std::integral_constant<bool, !B> {};
